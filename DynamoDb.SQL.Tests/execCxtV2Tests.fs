...
type ``Given a V2 DynamoQuery`` () =
    ...
    member this.``when there is only an equality filter then Filter should contain a single key condition`` () =
        let (GetQueryConfig config) = parseDynamoQueryV2 "SELECT * FROM Employees WHERE FirstName = \"Yan\""
    ...
    member this.``when a number of attributes were specified in the SELECT clause then they should be captured in AttributesToGet and Select should be set to SpecificAttributes`` () =
        let (GetQueryConfig config) = parseDynamoQueryV2 "SELECT FirstName, LastName, Age FROM Employees WHERE FirstName = \"Yan\""
    ...
    member this.``when there are more than one filter condition specified then they should all be captured in Filter`` () =
        let (GetQueryConfig config) = parseDynamoQueryV2 "SELECT * FROM Employees WHERE FirstName = \"Yan\" AND Age BETWEEN 30 AND 40 And LastName BEGINS WITH \"C\""
        ...
        config.Filter.ToConditions().["LastName"].ComparisonOperator        |> should equal "BEGINS_WITH"

    [<Test>]
    member this.``when an Index option is specified with AllAttributes set to true then Select should be set to AllAttributes`` () =
        let (GetQueryConfig config) = parseDynamoQueryV2 "SELECT * FROM Employees WHERE FirstName = \"Yan\" WITH (INDEX(MyIndex, true))"
    ...
    member this.``when an Index option is specified with AllAttributes set to false then Select should be set to AllProjectedAttributes`` () =
        let (GetQueryConfig config) = parseDynamoQueryV2 "SELECT * FROM Employees WHERE FirstName = \"Yan\" WITH (INDEX(MyIndex, false))"
    ...
    member this.``when the ASC order is specified then BackwardSearch should be set to false`` () =
        let (GetQueryConfig config) = parseDynamoQueryV2 "SELECT * FROM Employees WHERE FirstName = \"Yan\" ORDER ASC"
    ...
    member this.``when the DESC order is specified then BackwardSearch should be set to true`` () =
        let (GetQueryConfig config) = parseDynamoQueryV2 "SELECT * FROM Employees WHERE FirstName = \"Yan\" ORDER DESC"
    ...
    member this.``when the NoConsistentRead option is specified then ConsistentRead should be set to false`` () =
        let (GetQueryConfig config) = parseDynamoQueryV2 "SELECT * FROM Employees WHERE FirstName = \"Yan\" WITH (NoConsistentRead)"
    ...
    member this.``when the QueryPageSize option is specified to be 5 then Limit should be set to 5`` () =
        let (GetQueryConfig config) = parseDynamoQueryV2 "SELECT * FROM Employees WHERE FirstName = \"Yan\" WITH (PageSize(5))"
    ...
    member this.``when the query is a Count query then it should except with NotSupportedException`` () =
        let query = parseDynamoQueryV2 "COUNT * FROM Employees WHERE FirstName = \"Yan\""
