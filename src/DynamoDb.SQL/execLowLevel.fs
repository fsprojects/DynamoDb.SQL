...
module LowLevel =
    let (|GetQueryReq|) (query : DynamoQuery) = 
        ...
        | { From    = From table
            ...
            req.Select <- 
                match isCount, attributes, allAttributes with
            ...
            keyConditions 
            |> List.iter (fun (attrName, keyCond) -> 
                req.KeyConditions.Add(attrName, keyCond.ToCondition()))
            ...
            req.ReturnConsumedCapacity <- 
                match returnQueryConsumedCapacity opts with 
    ...
    let (|GetScanReqs|) (scan : DynamoScan) = 
        ...
        | { From    = From table
            ...
            | Some(Where(ScanCondition scanFilters)) -> 
                let scanFilters = 
                    scanFilters 
                    |> Seq.map (fun (attr, cond) -> attr, cond.ToCondition())
                req.ScanFilter <- new Dictionary<string, Condition>(dict scanFilters)
            | _ -> ()

            req.ReturnConsumedCapacity <- 
                match returnScanConsumedCapacity opts with 
                | true -> ReturnConsumedCapacity.TOTAL
                | _    -> ReturnConsumedCapacity.NONE
            req.Limit <- 
    ...
    let mergeQueryResponses (res : QueryResponse) (aggrRes : QueryResponse option) = 
        ...
        | Some aggr ->
            ...
            // in the V2 API, we MIGHT NOT get any consumed capacity back if the 
            // NoReturnedCapacity query option is specified, so need to handle 
            // that case
    ...
    let mergeScanResponses maxResults (aggrRes : ScanResponse) (res : ScanResponse) = 
        ...
        | null -> ()
        | lst  -> 
            // respect the max number of results we want to return from this 
            // new response IEnumerable.Take is used here instead of Seq.take 
            // because Seq.take excepts when there are insufficient number of 
            // items which is not desirable here
    ...
    let rec queryLoop (client : AmazonDynamoDBClient) maxResults (req : QueryRequest) (aggrRes : QueryResponse option) =
        async {
            // the request limit (max number of items to return) should not exceed 
            // the max number of results we want to return in total
    ...
    /// NOTE: there are subtle differences with the way 'Limit' works in Scan and 
    /// Query operations, hence why the two separate loop functions.
    /// In short, 'Limit' in Query terms defines the max number of items to return, 
    /// but in Scan terms it defines the max number of items to evaluate (page size)
    ...
    let rec scanLoop (client : AmazonDynamoDBClient) maxResults (aggrRes : ScanResponse option) (req : ScanRequest) =
        async {
            // don't set the limit using the maxResults for a scan, because unlike 
            // a query, limit is the max number of items scanned rather than max 
...
module ClientExt = 
    type AmazonDynamoDBClient with
        member this.QueryAsync (query : string) =
            ...
            | _ -> 
                let errMsg = sprintf "Not a valid query request : %s" query
                raise <| InvalidQueryException errMsg

        member this.Query (query : string) = 
            this.QueryAsync(query) |> Async.RunSynchronously
        ...
        member this.ScanAsync (query : string) =
            ...
            let scanReqs = 
                ...
                | _ ->
                    let errMsg = sprintf "Not a valid scan request : %s" query
                    raise <| InvalidScanException errMsg
            
            async {
                let! scanResponses = 
                    scanReqs 
                    |> Array.map (scanLoop this maxResults None) 
                    |> Async.Parallel
                return scanResponses 
                       |> Seq.reduce (mergeScanResponses maxResults)
            }
            
        member this.Scan (query : string) = 
            this.ScanAsync(query) |> Async.RunSynchronously
...
type AmazonDynamoDBClientExt =
    ...
    static member QueryAsyncAsTask (clt : AmazonDynamoDBClient, query : string) = 
        clt.QueryAsync(query) |> Async.StartAsTask
    ...
    static member ScanAsyncAsTask (clt : AmazonDynamoDBClient, query : string) =
        clt.ScanAsync(query) |> Async.StartAsTask
